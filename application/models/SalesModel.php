...
class SalesModel extends CI_Model
{
  ...
  function getCustomers($ACCode){
    ...
    $sql ="
            ...
            Where PartyMaster.BrokerCode = ACMaster.ACCode
              ...
              AND ACMaster.WorkYear = '$WorkYear'
              and replace(PartyMaster.PartyName, ' ', '') like '$ACCode%'
  ...
  function godownwise($gid) {
    ...
    $sql="
              ...
              where dbView_StockLedger.CoID = '$CoID'
                ...
                and dbView_StockLedger.GodownID = '$gid'
  ...
  function lotwise($gid, $LotNo) {
    ...
    $sql="
              ...
              where dbView_StockLedger.CoID = '$CoID'
                and dbView_StockLedger.WorkYear = '$WorkYear'
                and dbView_StockLedger.GodownID = '$gid'
                and dbView_StockLedger.LotNo = '$LotNo'
                and dbView_StockLedger.BalQty > 0 
                order by Cast(LotNo as Integer)                
  ...
  function itemwise($gid, $itemCode)
  {
    ...
    $sql="
              ...
              where dbView_StockLedger.CoID = '$CoID'
                and dbView_StockLedger.WorkYear = '$WorkYear'
                and dbView_StockLedger.GodownID = '$gid'
                and dbView_StockLedger.ItemCode = '$itemCode'
                and dbView_StockLedger.BalQty > 0 
                order by Cast(LotNo as Integer)                
         ";
    $query = $this->db->query($sql);
  ...
  function loadSalesDetail($id){
    ...
    SELECT
          ...
          SaleDetails.TaxCode,
          SaleDetails.BillText1,
          SaleDetails.BillText2,
          SaleDetails.NBrokRate,
          SaleDetails.WgtDiff, 
    ...
    // }
        ...
        $sql = "
                      select 
                            ...
                            sm.BillAmt                
                      from
                      SaleMast sm, SaleDetails sd, ACMaster ac, PartyMaster pm, ItemMaster
                      where sm.CoID=sd.CoID 
                            ...
                            AND sd.ItemCode = ItemMaster.ItemCode
                            and sd.CoID = ItemMaster.CoID
                            and sd.WorkYear = ItemMaster.WorkYear
  ...
  // updated 13-02-21
  function x_270521_get_SalesDatewise(){
    $CoID = $this->session->userdata('CoID') ;
    $WorkYear = $this->session->userdata('WorkYear') ; 
  ...
  // updated 13-02-21
  function x_270521_get_SalesDatewiseFilter($fromYear,$toYear){
    $CoID = $this->session->userdata('CoID') ;
    $WorkYear = $this->session->userdata('WorkYear') ; 
  ...
  function x_godownWise_170521($gid){
    $CoID = $this->session->userdata('CoID') ;
    $WorkYear = $this->session->userdata('WorkYear');
  ...
  // created - 23/03/21
  function x_lotWise_170521($gid,$LotNo){
    $CoID = $this->session->userdata('CoID') ;
    $WorkYear = $this->session->userdata('WorkYear');
  ...
  function X_itemWise_170521($gid,$itemCode){
    $CoID = $this->session->userdata('CoID') ;
    $WorkYear = $this->session->userdata('WorkYear');
