...
class SalesController extends CI_Controller
{
  ...
  public function show()
  {
    if ($this->input->post('submit') != NULL) {
      ...
      // echo 'postdata '. $fromYear;
      // echo 'postdata '. $toYear;
      $sessiondata['salesfromyear'] = $fromYear;
      $sessiondata['salestoyear'] = $toYear;
      ...
      if ($this->session->userdata('salesfromyear') != '') {
        ...
        $fromYear = $this->session->userdata('salesfromyear');
        $toYear   = $this->session->userdata('salestoyear');
        $this->load->model('SalesModel');
        $data['Item_List'] = $this->SalesModel->get_detailsFilter($fromYear, $toYear);
      } else {
  ...
  public function InsertSalesTry()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      // 190521 $taxable = $this->input->post('Taxable');
      // 300521 $taxable = $this->input->post('NetAmt');
      ...
      $godownID = $this->input->post('GodownID');
      $partyCode = $this->input->post('PartyCode');
      // 190521 $taxable = round($this->input->post('Taxable') + $apmcchg,2);
      $taxable = round(($taxable + $apmcchg), 2);
      $tax = $this->input->post('TaxRate');
      ...
      $data1 = array(
        ...
        'BillText1' => $this->input->post('BillText1'),
        'BillText2' => $this->input->post('BillText2'),
        'NBrokRate' => $this->input->post('NBrokRate'),
  ...
  public function EditTry($billid)
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $add = $salesDetailsTotal[0]->TCSAmount + $salesDetailsTotal[0]->TaxableAmt +
        $salesDetailsTotal[0]->IGSTAmt + $salesDetailsTotal[0]->CGSTAmt + $salesDetailsTotal[0]->SGSTAmt;
      $billAmtTotal = $add + $this->input->post('HelMajuri') + $this->input->post('OtherChrgs');
  ...
  function InsertDetail($bill, $gid)
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $salesDetailsTotal = $this->SalesModel->getTotal($bill);
